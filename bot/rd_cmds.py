...
def dice(expression):
    ...
    try:
        ...
        allowed_names = {
            'str': str, 'int': int, 'float': float, 'list': list, 'bool': bool,
            'abs': abs, 'min': min, 'max': max, 'map': map, 'range': range,
            'ord': ord, 'chr': chr,
            'bin': bin, 'hex': hex, 'oct': oct,
        }
        allowed_names.update({k: getattr(math, k) for k in dir(math) if not k.startswith("_")})
        allowed_names.update({k: getattr(random, k) for k in dir(random) if not k.startswith("_")})

        final_result = eval(compile(eval_expression, "<string>", "eval"), safe_globals, allowed_names)
        ...
        if not with_d:
            final_expression = ""
        if len(f"{final_expression}{final_result}") <= 500:
            return f"{final_expression}{final_result}"
        else:
            return f"{eval_expression}={final_result}"
    except PermissionError as e:
        print("[禁止执行] 存在被限制的函数或符号:", e, end = '')
...
def run_rd_cmds(l):
    if l[0] == "信息" or l[0] == "info":
        ...
        print("#rd info   程序简介")
        print("#rd <智能算式(长度至少为3)>")
        print("支持执行丢骰子、math和random库全部函数的调用，格式例如：d5*(3d6+2)+sin(pi/2)")
