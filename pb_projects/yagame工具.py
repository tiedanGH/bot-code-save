...
def fetch_rooms_info(storage_data, room_id_filter=None):
    ...
    def on_open(ws):
        ...
        ws_connection = ws
        # 发送初始连接请求
        ws.send(json.dumps({"id": hall_room_id, "keyID": "VISITOR"}))
            ...
            for response in ws_responses:
                if response.get("type") == "ok" and response.get("msg", {}).get("type") == "rooms":
                    if ws_connection:
                        ws_connection.close()
                    if room_id_filter:
                        return process_room_detail(response, room_id_filter)
                    else:
...
def process_rooms_response(response):
    ...
    if not public_rooms:
        ...
        for index, room in enumerate(public_rooms, 1):
            room_id = room.get("id", "")
            ...
            <div class="room-item">
                <div class="room-header">
                    <span class="room-index">{index}</span>
...
def process_room_detail(response, room_index):
    """处理指定编号的房间的详细信息"""
    rooms_data = response.get("msg", {}).get("rooms", [])
    ...
    # 检查编号是否有效
    if not public_rooms:
        return { "content": f"当前没有可加入的游戏房间" }
    if room_index < 1 or room_index > len(public_rooms):
        return { "content": f"[错误] 编号无效，请输入1~{len(public_rooms)}之间的数字" }
    
    # 获取指定编号的房间
    target_room = public_rooms[room_index - 1]
    room_id = target_room.get("id", "")
    
    # 提取房间信息
    room_name = target_room.get("name", "[未知游戏]")
    players = target_room.get("players", [])
    player_count = len(players)
    player_names = [player.get("user", "") for player in players]
    
    # 确定房间状态
    if target_room.get("ended", False):
        status = "已结束"
    elif target_room.get("started", False):
        status = "进行中"
    else:
        status = "等待中"
    
    # 生成网页链接
    web_url = f"https://yagame.tiedan.site/app/game/client.html?id={room_id}&public=1"
    
    # 构建文本输出
    text_content = f"""游戏名称：{room_name}
房间ID：{room_id}
房间状态：{status}
玩家数量：{player_count}人
玩家列表：{', '.join(player_names) if player_names else '无'}
房间链接：
{web_url}"""
...
def main():
    ...
    if not input_parts:
        ...
        if command in ['help', 'h', '帮助']:
            output = get_help_text()
        elif command in ['commit', '提交']:
            output = fetch_github_commits()
        elif command in ['about', '关于']:
            output = get_about_text()
        elif command in ['room', 'rooms', '房间']:
            if len(input_parts) > 1:
                # 带参数的room命令：获取指定房间详情
                try:
                    room_index = int(input_parts[1])
                    output = fetch_rooms_info(storage_data, room_index)
                except ValueError:
                    output = {"content": "[错误] 房间编号必须是数字"}
            else:
                # 无参数的room命令：获取房间列表
                output = fetch_rooms_info(storage_data)
        elif command == 'admin' and len(input_parts) >= 3:
            # 处理admin命令
            if storage_data['userID'] in ADMINS:
                subcommand = input_parts[1].lower()
                value = ' '.join(input_parts[2:]).strip()
                
                if subcommand == 'server':
                    if value:
                        output = update_global_storage(storage_data, "server_address", value)
                    else:
                        output = {"content": "[错误] 服务器地址不能为空"}
                elif subcommand == 'room':
                    if value:
                        output = update_global_storage(storage_data, "room_id", value)
                    else:
                        output = {"content": "[错误] YaGameHall ID 不能为空"}
                else:
                    output = {"content": "[错误] 未知的管理命令"}
            else:
                output = {"content": "[错误] 未持有管理员权限"}
        else:
            output = {"content": "[错误] 未知指令，请使用「##yagame help」查看指令帮助"}
